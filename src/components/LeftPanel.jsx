    ...
    <div className="left-panel">
      <div className="concept-header">
        ...
        <div className="prof-display">
          <div className={`prof-pct ${pct >= 70 ? "prof-done" : "prof-active"}`}>
            {pct}%
          </div>
      ...
      <div className="options-list">
        {problem.options.map((opt, i) => {
          ...
          if (answered) {
            ...
            else if (i === selected && i !== problem.correct) cls += " option-wrong";
            else cls += " option-dim";
          }
          return (
            <button
            ...
            >
              ...
              <span className="option-text">{typeof opt === "object" ? opt.text : opt}</span>
              {answered && i === problem.correct && (
                <svg className="option-icon" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
      ...
      {feedback && (
        <div className={`feedback-box feedback-${feedback.type}`}>
          <div className="feedback-header">
            {feedback.type === "ok" ? (
              <>
                <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" aria-hidden="true"><circle cx="12" cy="12" r="10"/><polyline points="9 12 11 14 15 10"/></svg>
                Correto!
              </>
            ) : (
              <>
                <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" aria-hidden="true"><circle cx="12" cy="12" r="10"/><line x1="15" y1="9" x2="9" y2="15"/><line x1="9" y1="9" x2="15" y2="15"/></svg>
                Incorreto
              </>
            )}
          </div>
          ...
          <button className="btn-next" onClick={onNext}>
            {problemIdx + 1 < concept.problems.length ? "Próximo problema" : "Próximo nó"}
