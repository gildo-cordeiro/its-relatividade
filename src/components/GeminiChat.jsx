...
export default function GeminiChat({ concept, problem, problemIdx, selectedOptionIdx, onPenalty }) {
  ...
  async function send() {
    ...
    const selOpt = selectedOptionIdx !== null && selectedOptionIdx !== undefined ? problem.options[selectedOptionIdx] : null;
    const misconception = (selOpt && typeof selOpt === "object") ? selOpt.misconception : null;

    const systemPrompt = buildTutorPrompt(concept, problem, misconception);

    const optionsText = problem.options.map((o, i) => `(${i + 1}) ${typeof o === "object" ? o.text : o}`).join(", ");
    ...
    if (selectedOptionIdx !== null && selectedOptionIdx !== undefined) {
      ...
      const correctText = typeof correctOpt === "object" ? correctOpt.text : correctOpt;
  ...
  return (
    <div className="gemini-chat">
      ...
      <div className="chat-messages">
        {messages.map((m, i) => (
          <div key={i} className={`chat-msg chat-msg-${m.role === "error" ? "error" : m.role}`}>
            {(m.role === "assistant" || m.role === "error") && (
              <div className={`chat-avatar ${m.role === "error" ? "chat-avatar-err" : ""}`}>
                {m.role === "error" ? "!" : "G"}
              </div>
            )}
            <div className={`chat-bubble ${m.role === "error" ? "chat-bubble-err" : ""}`}>
              <ReactMarkdown
                ...
                components={{
                  ...
                  ul: ({ node, ...props }) => <ul style={{ listStylePosition: 'inside', paddingLeft: '0.5rem' }} className="list-disc mb-2 space-y-1" {...props} />,
                  ol: ({ node, ...props }) => <ol style={{ listStylePosition: 'inside', paddingLeft: '0.5rem' }} className="list-decimal mb-2 space-y-1" {...props} />,
